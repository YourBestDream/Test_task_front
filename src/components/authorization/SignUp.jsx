...
export default function SignUp() {
  ...
  const [passworRepeatdVisible, setPasswordRepeatVisible] = useState(false);
  ...
  const handlePasswordRepeatVisibility = () => {
    setPasswordRepeatVisible(!passworRepeatdVisible);
  };
  ...
  const onSubmit = async (values) => {
    ...
    try {
      ...
      // Redirect or handle the response upon successful signup
      navigate('/home'); // Redirect to login page or dashboard as needed
    } catch (error) {
      if (error.response) {
  ...
  return (
    <div className={`${styles.authorization__container} ${styles.container}`}>
      <div className={styles.authorization__wrapper}>
        ...
        <span className="text-md">Discover the power of AI with us</span>
        <Form
          onSubmit={onSubmit}
          render={({ handleSubmit }) => (
            <form
            ...
            >
              ...
              <div className={styles.passwordInputWrapper}>
                ...
                <button
                  type="button"
                  onClick={handlePasswordRepeatVisibility}
                  className={styles.passwordVisibilityToggle}
                  tabIndex={-1}
                >
                  {passworRepeatdVisible ? (
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      className={styles.icon}
                      viewBox="0 0 18 15"
        ...
        />
        <button className={`${styles.authorization__toggle} text-sm`} type="button" onClick={() => navigate('/login')}> Already have an account? Log In </button>
      </div>
    </div>
  );
}
